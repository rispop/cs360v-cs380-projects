...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        /* TCP is a byte stream: one command can arrive split across several
         * reads, so read until the terminating newline (or EOF). */
        char line[LINE_MAX_LEN];
        size_t got = 0;
        while (got < sizeof line - 1) {
            ssize_t n = read(c, line + got, sizeof line - 1 - got);
            if (n <= 0) break;                        /* EOF, or error */
            got += (size_t)n;
            if (memchr(line, '\n', got)) break;       /* the command is complete */
        }
        if (got > 0) {
            line[got] = '\0';
            char reply[512];
